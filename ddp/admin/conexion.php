...
class conexion
{
    ...
    public function __construct()
    {
        ...
        $database = getenv('DDP_DB_NAME') ?: 'revista_digital2';
    ...
    public function obtenerReportajes(): array
    {
        $consulta = $this->db->prepare(
    ...
    public function obtenerPodcasts(): array
    {
        $consulta = $this->db->prepare(
    ...
    public function obtenerPodcast(int $id): ?array
    {
        $consulta = $this->db->prepare(
            'SELECT id, titulo, url_embed, portada, estado, fecha_publicacion
             FROM podcasts
    ...
    public function obtenerNoticia(int $id): ?array
    {
        $consulta = $this->db->prepare(
            'SELECT id, titulo, foto, link_externo, estado, fecha_publicacion
    ...
    public function obtenerVideos(): array
    {
        $consulta = $this->db->prepare(
            'SELECT id, titulo, url_embed, portada, estado, fecha_publicacion
             FROM videos
    ...
    public function obtenerVideo(int $id): ?array
    {
        $consulta = $this->db->prepare(
            'SELECT id, titulo, url_embed, portada, estado, fecha_publicacion
    ...
    public function guardarVideo(array $datos, int $usuarioId, ?int $id = null): int
    {
        if ($id === null) {
            $consulta = $this->db->prepare(
                'INSERT INTO videos (titulo, url_embed, portada, estado, fecha_publicacion, usuario_id)
                 VALUES (?, ?, ?, ?, ?, ?)'
            );
            $consulta->bind_param(
                'sssssi',
    ...
    public function obtenerBoletines(): array
    {
        $consulta = $this->db->prepare(
    ...
    public function eliminarContenido(string $tipo, int $id): void
    {
        $tablas = [
        ...
        $consulta = $this->db->prepare('DELETE FROM ' . $tablas[$tipo] . ' WHERE id = ?');
        $consulta->bind_param('i', $id);
        $consulta->execute();
        $consulta->close();
    }

    public function obtenerBoletin(int $id): ?array
    {
        $consulta = $this->db->prepare(
            'SELECT id, numero_boletin, titulo_boletin, resumen, foto_portada, archivo_pdf, estado, fecha_publicacion
             FROM boletines WHERE id = ? LIMIT 1'
        );
        $consulta->bind_param('i', $id);
        $consulta->execute();
    ...
    public function crearReportaje(array $datos, int $usuarioId): void
    {
        ...
        public function obtenerReportaje(int $id): ?array
        {
            $consulta = $this->db->prepare(
            ...
            try {
                if ($id === null) {
                    ...
                    $reportaje->bind_param(
                        ...
                        $datos['foto_principal'], $datos['pdf_adjunto'], $datos['estado'],
                    ...
                    $reportaje = $this->db->prepare(
                        'UPDATE reportajes
                         SET titulo = ?, resumen_corto = ?, desarrollo = ?, foto_principal = ?,
                             pdf_adjunto = ?, estado = ?, fecha_publicacion = ?, es_destacado = ?
                         WHERE id = ?'
                    );
                    $reportaje->bind_param(
                        'sssssssii',
